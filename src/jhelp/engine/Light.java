   ...
   public void render(final GL gl)
   {
      if(this.asChanged == true)
      {
         ...
         if(this.needRefresh == true)
         {
            ...
            gl.glLightfv(this.id, GL.GL_AMBIENT, this.ambiant.putInFloatBuffer());
            gl.glLightfv(this.id, GL.GL_DIFFUSE, this.diffuse.putInFloatBuffer());
            gl.glLightfv(this.id, GL.GL_SPECULAR, this.specular.putInFloatBuffer());

            gl.glLightfv(this.id, GL.GL_POSITION, BufferUtils.transferFloat(this.x, this.y, this.z, this.w));

            gl.glLightfv(this.id, GL.GL_SPOT_DIRECTION, BufferUtils.transferFloat(this.spotDirection.x, this.spotDirection.y, this.spotDirection.z));
            gl.glLighti(this.id, GL.GL_SPOT_EXPONENT, this.spotExponent);
            gl.glLighti(this.id, GL.GL_SPOT_CUTOFF, this.spotCutOff);

            gl.glLightf(this.id, GL.GL_CONSTANT_ATTENUATION, this.constantAttenuation);
            gl.glLightf(this.id, GL.GL_LINEAR_ATTENUATION, this.linearAttenuation);
