...
public class Node
{
     ...
     *           OpenGL context
     */
    void matrixRootToMe(final GL gl)
    {
    ...
    protected void fireMouseClick(final boolean leftButton, final boolean rightButton)
    {
        synchronized (this.arrayListListners)
        {
            ...
            {
                ThreadManager.THREAD_MANAGER.doThread(this.taskFireMouseClick,
                                                      new Triplet<NodeListener, Boolean, Boolean>(nodeListener, leftButton,
                                                                                                  rightButton));
            }
        }
    }
     ...
     *           OpenGL utilities
     */
    synchronized void renderPickingUV(final Node node, final GL gl, final GLU glu)
    {
        gl.glPushMatrix();
        this.matrix(gl);
     ...
     *           Scene renderer
     */
    synchronized void renderTheNodePicking(final GL gl, final GLU glu, final JHelpSceneRenderer sceneRenderer)
    {
        gl.glPushMatrix();
        this.matrix(gl);
     ...
     *           OpenGL utilities
     */
    protected void renderSpecificPicking(final GL gl, final GLU glu)
    {
    }
     ...
     *           OpenGL utilities
     */
    protected void renderSpecificPickingUV(final GL gl, final GLU glu)
    {
    }
     ...
     *
     * @param additionalInformation New developper addional information
     */
    ...
    public final boolean isMePick(final float red, final float green, final float blue)
    {
        ...
        return Math3D.equalPick(red, this.redPicking) && Math3D.equalPick(green, this.greenPicking) && Math3D.equalPick(
                blue, this.bluePicking);
    }
