      ...
      implements GLEventListener, MouseListener, MouseMotionListener, MouseWheelListener, KeyListener, Runnable
{
    ...
    *           OpenGL context
    */
   private void computeModelView(final GL gl)
   {
      ...
      gl.glGetDoublev(GL.GL_MODELVIEW_MATRIX, BufferUtils.TEMPORARY_DOUBLE_BUFFER);
    ...
    *           OpenGL context
    */
   private void computeProjection(final GL gl)
   {
      ...
      // Get projection
      BufferUtils.TEMPORARY_DOUBLE_BUFFER.rewind();
      gl.glGetDoublev(GL.GL_PROJECTION_MATRIX, BufferUtils.TEMPORARY_DOUBLE_BUFFER);
    ...
    *           OpenGL context
    */
   private void computeViewPort(final GL gl)
   {
      ...
      gl.glGetIntegerv(GL.GL_VIEWPORT, BufferUtils.TEMPORARY_INT_BUFFER);
    ...
    *           GLU context
    */
   private void drawOver3D(final GL gl, final GLU glu)
   {
    ...
    *           GLU context
    */
   private void drawUnder3D(final GL gl, final GLU glu)
   {
    ...
    */
   private Point2D gluProject(final float x, final float y, final float z, final GL gl, final GLU glu)
   {
    ...
    */
   private Point3D gluUnProject(final float x, final float y, final float z, final GL gl, final GLU glu)
   {
      this.computeModelView(gl);
      this.computeProjection(gl);
      this.computeViewPort(gl);
   ...
   private void makeSnapShot(final GL gl, final boolean invert)
   {
      ...
      if(this.makeAScreenShot == true)
      {
         ...
         gl.glReadPixels(0, 0, this.width, this.height, GL.GL_RGBA, GL.GL_FLOAT, BufferUtils.TEMPORARY_FLOAT_BUFFER);
         BufferUtils.TEMPORARY_FLOAT_BUFFER.rewind();
    ...
    *           Y
    */
   private void pickColor(final GL gl, final int x, final int y)
   {
      ...
      // Get picking color
      BufferUtils.TEMPORARY_FLOAT_BUFFER.rewind();
      gl.glReadPixels(x, this.height - y, 1, 1, GL.GL_RGBA, GL.GL_FLOAT, BufferUtils.TEMPORARY_FLOAT_BUFFER);
      BufferUtils.TEMPORARY_FLOAT_BUFFER.rewind();
    ...
    *           OpenGL context
    */
   private void playAnimations(final GL gl)
   {
    ...
    *           OpenGL context
    */
   private void prepareMaterial2D(final GL gl)
   {
    ...
    *           Height
    */
   private void show2D(final GL gl, final GLU glu, final Texture texture, final int x, final int y, final int width, final int height)
   {
    ...
    */
   void drawPickHotspot(final GL gl, final GLU glu, final Node node, final float red, final float green, final float blue)
   {
    ...
    *           Actual camera
    */
   void renderPicking(final GL gl, final GLU glu, final Camera camera)
   {
      ...
      if((this.detectX >= 0) && (this.detectX < this.width) && (this.detectY >= 0) && (this.detectY < this.height))
      {
         ...
         {
            // If no node detect, verify if a 2D object id detect
    ...
    *           Actual camera
    */
   void renderPickUV(final GL gl, final GLU glu, final Camera camera)
   {
      // Prepare for "picking rendering"
      gl.glDisable(GL.GL_LIGHTING);
      gl.glDisable(GL.GL_CULL_FACE);
    ...
    *           Node witch carry hotspot
    */
   void showHotspot(final GL gl, final GLU glu, final Node node)
   {
      final Texture textureHotspot = node.getTextureHotspot();
      // If no hotspot texture, do nothing
      if(textureHotspot == null)
   ...
   public void init(final GLAutoDrawable drawable)
   {
      ...
      BufferUtils.TEMPORARY_INT_BUFFER.rewind();
      gl.glGetIntegerv(GL.GL_MAX_LIGHTS, BufferUtils.TEMPORARY_INT_BUFFER);
      BufferUtils.TEMPORARY_INT_BUFFER.rewind();
   ...
   public void run()
   {
      ...
      while(this.thread != null)
      {
         ...
         {
            Debug.printMark(DebugLevel.VERBOSE, "PAUSE = " + this.pause);
