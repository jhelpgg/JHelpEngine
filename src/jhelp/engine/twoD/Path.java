...
public class Path
{
     ...
     */
    public void appendCubic(final Point2D startPoint, final Point2D controlPoint1, final Point2D controlPoint2,
     ...
     * @param end           End value
     */
    public void appendCubic(final Point2D startPoint, final float start, final Point2D controlPoint1, final float control1,
                            final Point2D controlPoint2,
                            final float control2, final Point2D endPoint, final float end)
    {
        this.path.add(new PathElement(PathType.CUBIC, new Point3D(startPoint, start), new Point3D(controlPoint1, control1),
     ...
     * @param end          End value
     */
    public void appendQuad(final Point2D startPoint, final float start, final Point2D controlPoint, final float control,
                           final Point2D endPoint, final float end)
    {
        this.path.add(new PathElement(PathType.QUAD, new Point3D(startPoint, start), new Point3D(controlPoint, control),
                                      new Point3D(endPoint, end)));
        this.size = -1;
    }
     ...
     * Bigger is the precision, more the path is smooth, but it takes more time to compute and more lines are computed,
     * so more
     * memory is used.<br>
     ...
     * Where :
     * <li><b>NB_LINES</b> : Numbers of line element</li>
     * <li><b>NB_QUADRICS</b> : Numbers of quadric element</li>
     * <li><b>NB_CUBICS</b> : Numbers of cubic element</li> <br>
    ...
    public void linearize(final float start, final float end)
    {
        ...
        for (final PathElement pathElement : this.path)
        {
            ...
            for (Point3D actualPoint : points)
            {
                if (oldPoint != null)
                {
                    dist = (float) Math.sqrt(Math3D.square(actualPoint.getX() - oldPoint.getX()) +
                                                     Math3D.square(actualPoint.getY() - oldPoint.getY()));
    ...
    public float computePathSize()
    {
        ...
        for (final PathElement pathElement : this.path)
        {
            ...
            {
                this.size += Math.sqrt(
                        Math3D.square(old.getX() - points[0].getX()) + Math3D.square(old.getY() - points[0].getY()));
            }
            for (int i = 1; i < length; i++)
            {
                this.size += Math.sqrt(Math3D.square(points[i - 1].getX() - points[i].getX()) + Math3D.square(
    ...
    /**
     * Start/select/end information to know where start/end an element when path is transformed on line array. select to
     * know who
